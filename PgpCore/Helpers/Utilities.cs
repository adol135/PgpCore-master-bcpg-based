...
namespace PgpCore
{
    ...
    public static class Utilities
    {
        ...
        {
            using (var decoder = PgpUtilities.GetDecoderStream(privateKeyStream))
            {
                var factory = new PgpObjectFactory(decoder);

                var secretRings = new List<PgpSecretKeyRing>();
                object obj;
                ...
                while ((obj = factory.NextPgpObject()) != null)
                {
                    ...
                    {
                        case PgpMarker _:
                            // Marker packet; ignore and continue
                            continue;

                        case PgpSecretKeyRingBundle bundle:
                            // If the file already contains a secret bundle, return it.
                            return bundle;

                        case PgpSecretKeyRing skr:
                            // Collect individual secret rings; we’ll build a bundle later.
                            secretRings.Add(skr);
                            break;

                        case PgpPublicKeyRing _:
                        case PgpPublicKeyRingBundle _:
                            // File is public-only; not valid for secret bundle usage
                            continue;

                        default:
                            // Other packets/objects can be skipped
                            break;
                    }
                }

                if (secretRings.Count == 0)
                    throw new PgpException("No PGP secret key ring found in the provided file.");

                // Build a bundle from the collected secret rings
                return new PgpSecretKeyRingBundle(secretRings);
            }
        }
